...
# Параметри Варіанта №4 (Готель) [cite: 483]
START_DATE = datetime(2025, 1, 1)
HOURS = 8760
PV_CAPACITY = 120  # кВт [cite: 483]
TOTAL_AVG_LOAD = 70  # кВт [cite: 483]
...
def generate_full_system_data():
    ...
    for h in range(HOURS):
        ...
        # 2. Споживання (Три рівні) [cite: 36, 44]
        daily_mod = 1 + 0.4 * np.sin(2 * np.pi * (hour - 6) / 24)
        seasonal_mod = 1.3 if month in [1, 2, 12, 6, 7, 8] else 1.0 #[cite: 82]
        total_kwh = TOTAL_AVG_LOAD * daily_mod * seasonal_mod * np.random.uniform(0.9, 1.1)
        
        # Розподіл по рівнях [cite: 59, 60]
        readings.append([1, ts, round(total_kwh, 3)]) # Рівень 1
        readings.append([2, ts, round(total_kwh * 0.3, 3)]) # Рівень 2 (Кухня)
        readings.append([3, ts, round(total_kwh * 0.5, 3)]) # Рівень 2 (Номери)
        readings.append([4, ts, round(total_kwh * 0.2, 3)]) # Рівень 2 (Технічний)
        ...
        # 3. СЕС та Батарея [cite: 240, 245]
        pv_gen = (irrad / 1000) * PV_CAPACITY * 0.8 #[cite: 243]
        soc = 50 + 20 * np.sin(2 * np.pi * hour / 24) #[cite: 286]
